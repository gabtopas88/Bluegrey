...
class MLPredictorStrategy(BaseStrategy):
    ...
    prediction_columns = ["prediction", "probability_-1", "probability_0", "probability_1"]
    ...
    def __init__(self, instruments: dict = None, params: dict = None):
        ...
        self.probability_threshold = self.params.get("probability_threshold", 0.6)
    ...
    def predict(self, df: pd.DataFrame, show_progress: bool = True) -> pd.DataFrame:
        ...
        # initialize empty DataFrame to store predictions and class probabilities
        predictions = pd.DataFrame(0.0, index=prediction_date_range, columns=self.prediction_columns)
        ...
        for i, prediction_date in enumerate(prediction_date_range):
            ...
            # Predict each class probability and select the most probable class.
            probabilities = model.predict_proba(X_predict)[0]
            classes = model.classes_.astype(int)
            predictions.loc[prediction_date, "prediction"] = classes[probabilities.argmax()]
            predictions.loc[prediction_date, [f"probability_{label}" for label in classes]] = probabilities
    ...
    def generate_signals(self, df: pd.DataFrame = None, show_progress: bool = True) -> pd.DataFrame:
        ...
        # Assign a weight only when the largest class probability clears the threshold.
        weights = pd.DataFrame(index=predictions.index)
        weights[self.symbol] = predictions["prediction"].where(
            predictions.filter(like="probability_").max(axis=1) > self.probability_threshold, 0
        ) * self.position_size
    ...
    def predict_single_event(self, df: pd.DataFrame, prediction_date: pd.Timestamp) -> pd.Series:
        ...
        # Skip this date if train_start is earlier than first date in dataset.
        if train_start < df.index[0]:
            return pd.Series(0.0, index=self.prediction_columns)
        ...
        probabilities = model.predict_proba(X_predict)[0]
        classes = model.classes_.astype(int)
        prediction = pd.Series(0.0, index=self.prediction_columns)
        prediction["prediction"] = classes[probabilities.argmax()]
        prediction[[f"probability_{label}" for label in classes]] = probabilities

        return prediction
    


    def walk_forward_predictions(self, df: pd.DataFrame, show_progress: bool = True) -> pd.DataFrame:
        ...
        for i, prediction_date in enumerate(prediction_date_range):
            ...
            # Update the predictions DataFrame for this date.
