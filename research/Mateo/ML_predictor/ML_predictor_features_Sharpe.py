...
# Plotting feature-selection diagnostics.
...
class MLPredictorStrategy(BaseStrategy):
    ...
    def __init__(self, instruments: dict = None, params: dict = None):
        ...
        # Feature-selection diagnostics: stores which features SelectKBest passes to MLP each day.
        self.feature_diagnostics_enabled = self.params.get("feature_diagnostics_enabled", True)
        self.feature_diagnostics_top_n = self.params.get("feature_diagnostics_top_n", self.n_features)

        # These are populated during predict() / walk_forward_predictions().
        self.selected_features_by_date_ = {}
    ...
    def record_feature_selection( self, prediction_date: pd.Timestamp, model: Pipeline, X_train: pd.DataFrame) -> None:

        '''
        Store the features selected by SelectKBest for one prediction date,
        together with their absolute Pearson-correlation scores.
        '''
        ...
        # Recover SelectKBest scores and selected features.
        selector = model.named_steps["selector"]
        selector_scores = pd.Series(
            selector.scores_,
            index=kept_after_variance,
            name="pearson_abs_corr",
        ).replace([np.inf, -np.inf], np.nan)

        selected_mask = selector.get_support()
        selected_features = selector_scores.index[selected_mask].tolist()

        # Store the exact feature list passed through to the MLP on this date.
        self.selected_features_by_date_[prediction_date] = selected_features
        self.selected_features_list_ = [
            {"prediction_date": date, "features": features}
            for date, features in self.selected_features_by_date_.items()
        ]

        # Store top-N ranked features, marking which ones were actually selected.
        ranked_scores = selector_scores.sort_values(ascending=False)
        if self.feature_diagnostics_top_n is not None:
            ranked_scores = ranked_scores.head(int(self.feature_diagnostics_top_n))

        selected_set = set(selected_features)

        for rank, (feature, score) in enumerate(ranked_scores.items(), start=1):
            self.feature_selection_records_.append({
                "prediction_date": prediction_date,
                "feature": feature,
                "pearson_abs_corr": score,
                "rank": rank,
    ...
    def plot_feature_frequency( self, top_n: int = 30, selected_only: bool = True, start_date: str | pd.Timestamp = None, end_date: str | pd.Timestamp = None):
        ...
        '''
        Plot how frequently features appeared in the per-date feature-selection diagnostics.
        '''
        ...
        # Optionally restrict to selected features and/or a date window.
        if selected_only:
        ...
        # Count how often each feature appears, then plot most frequent features.
    ...
    def predict(self, df: pd.DataFrame, show_progress: bool = True, prediction_start_date: str | pd.Timestamp = None) -> pd.Series:
        ...
        for i, prediction_date in enumerate(prediction_date_range):
            ...
            # Record which features SelectKBest passed through to the MLP for this date.
            self.record_feature_selection(prediction_date, model, X_train)
